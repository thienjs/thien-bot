...
module.exports = class ThienClient extends CommandoClient {
	constructor(options) {
		super(options);
		...
		this.webhook = new WebhookClient(THIEN_WEBHOOK_ID, THIEN_WEBHOOK_TOKEN, { disableMentions: 'everyone' });
	...
	isBlockedFromPhone(origin, recipient, caller) {
		return (recipient.guild && recipient.topic.includes(`<thien:phone:block:${origin.id}>`))
			|| (recipient.guild && recipient.topic.includes(`<thien:phone:block:${caller.id}>`))
			|| (origin.guild && recipient.guild && recipient.topic.includes(`<thien:phone:block:${origin.guild.id}>`))
			|| (origin.guild && origin.topic.includes(`<thien:phone:block:${recipient.id}>`))
			|| (origin.guild && recipient.guild && origin.topic.includes(`<thien:phone:block:${recipient.guild.id}>`))
			|| (origin.guild && origin.topic.includes(`<thien:phone:block:${caller.id}>`));
	}
